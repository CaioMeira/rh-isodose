...
#    available at https://github.com/dicompyler/dicompyler-core/  https://www.dragonflybsd.org/~beket/rt.py
...
from lib.dicompylercore import dvh, util
from lib.dicompylercore.config import pil_available, shapely_available
...
logger = logging.getLogger('lib.dicompylercore.dicomparser')
...
############################## RT Dose Methods ###############################
    ...
    def GetRawDVHs(self):
        """Returns RAW dose-volume histograms (DVHs)."""
        return self.ds.DVHSequence
    def AppendRawDVHs(self,dvhs):
        """Append RAW dose-volume histograms (DVHs)."""
        c = dvhs.ds.DVHSequence
        self.ds.DVHSequence.append(c)
        return dvhs

    def ReferenceRoiNumberEqual(self,dvhs):
        """Verify if ReferenceRoiNumber are equals """
        objOne =[]
        objTwo =[]
        if self.HasDVHs():
            for item in self.ds.DVHSequence:
                ...
                if not 'DVHReferencedROISequence' in item:
                    continue
                number = item.DVHReferencedROISequence[0].ReferencedROINumber
                objOne.append(number)

        if dvhs.HasDVHs():
            for item in dvhs.ds.DVHSequence:
                # Make sure that the DVH has a referenced structure / ROI
                if not 'DVHReferencedROISequence' in item:
                    continue
                number = item.DVHReferencedROISequence[0].ReferencedROINumber
                objTwo.append(number)

        return set(objOne) == set(objTwo)
