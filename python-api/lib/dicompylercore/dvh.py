...
import logging
import json
logger = logging.getLogger('lib.dicompyler.dvh')
            ...
            if 'DVHReferencedROISequence' in d:
                if 'ReferencedROINumber' in d.DVHReferencedROISequence[0]:
                    if roi_num == \
                        ...
                        break
        if sequence_num == -1:
            # ROI NÂO EXISTE DVH DATA
            print(roi_num)
            print(name)
            #se cair aqui tentar retirar o obketo com o nome name do rtstrucure
...
# ============================= DVH properties ============================= #
    ...
    def describe(self):
        """Describe a summary of DVH statistics in a json based format.
            https://thisrtplan.weebly.com/uploads/4/5/7/2/45723995/dvh_limit_table.pdf
            https://www.aapm.org/pubs/reports/rpt_101.pdf
        """
        # a Python object (dict):
        info = {
          "structure": format(self.name),
          "dvh_type": self.dvh_type,
          "volume": self.volume,
          "volume_und": self.volume_units,
          "dose_und": self.dose_units,
          "max_dose": self.max,
          "min_dose": self.min,
          "mean_dose": self.mean,
          "D95":format(self.D95),
          "D98":format(self.D98),
          "D100":format(self.D100),
          "D1cc":format(self.D1cc),
          "D2cc":format(self.D2cc),
          "D3cc":format(self.D3cc),
          "D4cc":format(self.D4cc),
          "D5cc":format(self.D5cc),
          "D6cc":format(self.D6cc),
          "D10cc":format(self.D10cc),
          "D15cc":format(self.D15cc),
          "D20cc":format(self.D20cc),
          "D65cc":format(self.D65cc),
          "D100cc":format(self.D100cc),
          "D180cc":format(self.D180cc),
          "D200cc":format(self.D200cc),
          "D700cc":format(self.D700cc),
          "D1000cc":format(self.D1000cc),
          "D1500cc":format(self.D1500cc),

        }
        if self.dose_units == relative_units:
            info.update({"V100":format(self.V100)})
            info.update({"V95":format(self.V95)})
            info.update({"V5":format(self.V5)})
